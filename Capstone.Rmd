...
This section will walk through the steps taken in order to create models to predict the likliehood of a patient experiencing a complication or death 30 days following an elective procedure based on underlying health conditions.
```{r}
#Beign Modeling process for predicting complications/morality rate based on patients underlying health conditions
...
Define the two models, one for complication and one for mortality. These will contain the same predictor variables, but the response variable will be changed accordingly. 
```{r}
...
bestmodels=(1:3)[allcv10==min(allcv10)]
  ...
  if (bestmodels ==3) { #If the best model is greater than 20 , these refer to the logisitc regression models
...
bestmodels=(1:2)[allcv10==min(allcv10)]
  ...
  if (bestmodels ==2) { #If the best model is greater than 20 , these refer to the logisitc regression models
    model = "Logistic Regression"
  }
...
```
From the output, it can be seen that performing logisitc regression on the mortality model yielded the best model results. This model yields the lowest overall erro rrate of 13.24%. This model will be used to fit on the full dataset to get the most accurate coefficients for the predictor variables. By performing 1 layer of cross-validation, it can lead to underestimating the overall error rate of the model. To account for this underestimate, double cross-validation will be performed on the model selection process to validate our results. 
...
Random Forest Importance - Mortality
```{r}
#Random Forest - Complications Threshold Tuning
...
```
From the results above, when fitting the Random Forest model on the full dataset, and specifying a threshold of 5% to be considered a high risk of morality occuring, the models overall accuracy is 98.93% correct. In class accuracy is as follows:
